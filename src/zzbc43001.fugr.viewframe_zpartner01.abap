*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZPARTNER01
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZPARTNER01          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZPARTNER01'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZPARTNER01_EXTRACT
TOTAL = ZPARTNER01_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZPARTNER01'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZPARTNER01-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZPARTNER01_EXTRACT
TOTAL = ZPARTNER01_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZPARTNER01-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZPARTNER01'
...
TOTAL = ZPARTNER01_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZPARTNER01-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZPARTNER01-UPD_FLAG NE SPACE.
